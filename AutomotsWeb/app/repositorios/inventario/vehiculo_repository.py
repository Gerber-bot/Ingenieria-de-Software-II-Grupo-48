class VehiculoRepository:
    ...
    def buscar_vehiculos_para_venta(self, criterio="", limite=10):
        cursor = self.conn.cursor()
        ...
        cursor.execute("""
            ...
            WHERE 
                ...
                AND (
                    ...
                    OR v.vin LIKE ?
                    OR m.nombre LIKE ?
                    OR CAST(v.anio AS VARCHAR) LIKE ?
                    OR m.nombre + ' ' + v.modelo LIKE ?
                    OR v.modelo + ' ' + m.nombre LIKE ?
                )
            ORDER BY m.nombre, v.modelo, v.anio DESC
        """, (
            limite,
    ...
    def obtener_vehiculo_para_venta(self, id_vehiculo):
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT id_vehiculo, estado, precio
            FROM Vehiculo
            WHERE id_vehiculo = ?
        """, (id_vehiculo,))
